...
import { api } from '../api'
import { EVENT_TYPES, EVENT_ICONS, todayStr, formatDateJa, daysUntil } from '../util'
import { Modal, Field, inputClass, OshiSelect, PrimaryButton, Empty } from '../components/ui'
...
const emptyForm = { title: '', event_type: 'ライブ', event_date: '', oshi_id: null, memo: '', is_shared: false }
...
export default function Calendar() {
  ...
  const detailEvents = detailDate ? (byDate[detailDate] || []) : []
  ...
  return (
    <div className="space-y-3">
      ...
      {results ? (
        ...
        <>
          <p className="text-[11px] text-ink-soft text-center">日付をタップで選択、ダブルタップでその日の詳細</p>
          {months.map(({ year, month }) => {
            const isCurrent = year === new Date().getFullYear() && month === new Date().getMonth() + 1
            return (
              <div key={`${year}-${month}`} ref={isCurrent ? todayRef : null} className="bg-paper-card rounded-2xl border border-paper-line/60 p-3 scroll-mt-2">
                <p className="font-bold text-center text-ink mb-2">{year}年 {month}月</p>
                  ...
                  <li key={s.id} className="ml-4 relative">
                    ...
                    <div className="bg-paper rounded-xl p-3">
                      ...
                      <p className="text-[11px] text-ink-soft mt-0.5">
                        ...
                        {s.is_shared && s.is_own && <span className="ml-1 text-wine">🔗 共有中</span>}
                      </p>
                      {s.memo && <p className="text-[11px] text-ink-soft mt-1">{s.memo}</p>}
                      {s.is_own && (
                        <button onClick={() => remove(s)} className="text-[11px] text-ink-soft underline mt-1">削除</button>
                      )}
                    </div>
                  </li>
                ))}
