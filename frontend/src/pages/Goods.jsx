...
export default function Goods() {
  ...
  const save = async (e) => {
    ...
    setError('')
    try {
      await api('/goods', { method: 'POST', body: form })
      setForm(null)
      reload()
    } catch (err) {
  ...
  const remove = async (g) => {
    ...
    await api(`/goods/${g.id}`, { method: 'DELETE' })
    reload()
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="font-bold text-lg">グッズコレクション</h2>
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1">
        {['すべて', ...GOODS_CATEGORIES].map((c) => (
          <button key={c}
            className={`text-xs rounded-full px-3 py-1.5 border shrink-0 ${filter === c ? 'bg-pink-500 text-white border-pink-500' : 'bg-white border-pink-200 text-gray-600'}`}
            onClick={() => setFilter(c)}>
            {c}
          </button>
        ))}
      ...
      {filtered.length === 0 && (
        <Card>
          <Empty icon="🎁" message={'グッズがまだありません。\nアクスタやCDを登録してコレクションを作りましょう！'} />
        </Card>
      )}
        ...
        {filtered.map((g) => (
          <div key={g.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
            {g.image ? (
              ...
              <div className="w-full h-32 bg-pink-100 flex items-center justify-center text-4xl">🎁</div>
            )}
            <div className="p-2.5">
              <p className="font-bold text-sm truncate">{g.name}</p>
              <p className="text-[11px] text-gray-400 truncate">
                {g.category}
                {g.oshi_name ? `・${g.oshi_name}` : ''}
              </p>
              <div className="flex items-center justify-between mt-1">
                <span className="text-xs font-bold text-pink-600">
                  {g.price != null ? formatYen(g.price) : ''}
                </span>
                <button onClick={() => remove(g)} className="text-gray-300 text-sm px-1">×</button>
              </div>
            </div>
          </div>
        ))}
          ...
          <form onSubmit={save}>
            <Field label="グッズ名 *">
              <input className={inputClass} value={form.name} maxLength={50}
                ...
                placeholder="例：アクリルスタンド 2026ver." />
            </Field>
            <Field label="カテゴリ">
            ...
            <Field label="価格（円・任意）">
              <input type="number" min="0" className={inputClass} value={form.price}
                onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="例：1800" />
            </Field>
            <Field label="推し">
            ...
            {form.image && (
              <div className="flex items-center gap-3 mb-3">
                <img src={form.image} alt="プレビュー" className="w-16 h-16 rounded-xl object-cover" />
